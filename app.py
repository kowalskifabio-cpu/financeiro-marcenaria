...
# --- ABA 8: ANALISTA IA (CONSULTORIA ESTRATÉGICA) ---
with aba8:
    st.subheader("🤖 Analista IA - Consultoria Estratégica")
    st.markdown("Clique no botão abaixo para que a IA analise os dados filtrados nas abas de Indicadores e Curva ABC.")

    if st.button("🧠 Gerar Insight Estratégico", key="btn_ia_analise"):
        ...
        model = genai.GenerativeModel('models/gemini-1.5-flash') # Modelo rápido e eficiente
        ...
        df_bi, meses_ref = processar_bi(ano_sel, meses_sel, cc_sel)
        
        if df_bi is not None:
            # Resumo para a IA
            faturamento = df_bi[df_bi['Conta'].str.startswith('01') & (df_bi['Nivel'] == 2)]['ACUMULADO'].sum()
            despesa = df_bi[df_bi['Conta'].str.startswith('02') & (df_bi['Nivel'] == 2)]['ACUMULADO'].sum()
            lucro = faturamento + despesa
            rentabilidade = (lucro/faturamento*100) if faturamento > 0 else 0
            
            # Pega o Top 5 da Curva ABC (Despesas Analíticas)
            df_abc_ia = df_bi[(df_bi['Nivel'] == 4) & (df_bi['Conta'].str.startswith('02'))].copy()
            df_abc_ia['Abs'] = df_abc_ia['ACUMULADO'].abs()
            top5 = df_abc_ia.sort_values(by='Abs', ascending=False).head(5)
            lista_top5 = "\n".join([f"- {row['Descrição']}: {formatar_moeda_br(row['ACUMULADO'])}" for idx, row in top5.iterrows()])

            # 3. O Prompt (O Comando para a IA)
            prompt = f"""
            Aja como um Consultor Financeiro Sênior especializado em Marcenarias de alto padrão.
            Analise os seguintes dados financeiros do período {meses_ref} de {ano_sel}:
            
            - Faturamento Total: {formatar_moeda_br(faturamento)}
            - Despesa Total: {formatar_moeda_br(despesa)}
            - Lucro Líquido: {formatar_moeda_br(lucro)}
            - Rentabilidade: {rentabilidade:.1f}%
            
            As 5 maiores despesas (Curva ABC) são:
            {lista_top5}
            
            Por favor, forneça:
            1. Um diagnóstico rápido da saúde financeira.
            2. Alertas sobre os maiores gastos.
            3. Três ações práticas para melhorar o lucro no próximo mês.
            Seja direto, profissional e focado em resultados.
            """

            with st.spinner("🤖 O Analista está processando os números..."):
                try:
                    response = model.generate_content(prompt)
                    st.markdown("### 📋 Parecer do Consultor")
                    st.write(response.text)
                    st.info("💡 Nota: Esta análise é gerada por IA e deve ser validada com sua gestão estratégica.")
                except Exception as e:
                    st.error(f"Erro ao chamar a IA: {e}")
        else:
            st.warning("⚠️ Primeiro, gere os dados na aba 'Relatório' ou 'Indicadores' para que a IA tenha o que analisar.")
